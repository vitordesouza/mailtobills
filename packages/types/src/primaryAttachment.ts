...
function isPdfCandidate(candidate: AttachmentCandidateInput) {
...
function getReasonableSizeBonus(fileSize: number | undefined) {
  if (!fileSize || fileSize <= 0) return 0;

  const cappedSize = Math.min(fileSize, MAX_REASONABLE_PDF_SIZE_BYTES);
  return Math.min(
    Math.floor(cappedSize / SIZE_BUCKET_BYTES),
    MAX_SIZE_BONUS
  );
}

export function scoreAttachmentCandidate(
  candidate: AttachmentCandidateInput
...
export function choosePrimaryAttachment<T extends AttachmentCandidateInput>(
...
export function getAcceptedPdfAttachments<T extends AttachmentCandidateInput>(
  candidates: readonly T[]
): T[] {
  return candidates.filter((candidate) =>
    scoreAttachmentCandidate(candidate).isAcceptedPdf
  );
}
